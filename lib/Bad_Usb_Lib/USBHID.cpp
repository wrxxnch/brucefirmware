    ...
    for (uint8_t i = 0; i < tinyusb_loaded_hid_devices_num; i++) {
        ...
        if (device->device && device->reports_num) {
            for (uint8_t r = 0; r < device->reports_num; r++) {
                if (report_id == device->report_ids[r]) { return device->device; }
            }
        }
    }
    return NULL;
...
static uint16_t tinyusb_on_get_feature(uint8_t report_id, uint8_t *buffer, uint16_t reqlen) {
    ...
    if (device) { return device->_onGetFeature(report_id, buffer, reqlen); }
    return 0;
}
...
static uint16_t tinyusb_on_add_descriptor(uint8_t device_index, uint8_t *dst) {
    ...
    if (device->device) {
        ...
        if (res) {
            ...
            if (hid_report_map) {
                if (device->report_ids) { free(device->report_ids); }
...
static bool tinyusb_load_enabled_hid_devices() {
    if (tinyusb_hid_device_descriptor != NULL) { return true; }
    ...
    if (hid_report_map) {
        ...
        for (uint8_t i = 0; i < hid_report_map->reports_len; i++) {
            if (hid_report_map->reports[i].protocol_mode == ESP_HID_PROTOCOL_MODE_REPORT) {
                log_d(
                    "  ID: %3u, Type: %7s, Size: %2u, Usage: %8s",
...
extern "C" uint16_t tusb_hid_load_descriptor(uint8_t *dst, uint8_t *itf) {
    if (tinyusb_hid_is_initialized) { return 0; }
    ...
    uint8_t descriptor[TUD_HID_INOUT_DESC_LEN] = {
        ...
        // Interface number, string index, protocol, report descriptor len, EP OUT & IN address, size &
        // polling interval
...
uint8_t const *tud_hid_descriptor_report_cb(uint8_t instance) {
    ...
    if (!tinyusb_load_enabled_hid_devices()) { return NULL; }
...
void tud_hid_set_protocol_cb(uint8_t instance, uint8_t protocol) {
    ...
    arduino_usb_event_post(
        ...
        ARDUINO_USB_HID_SET_PROTOCOL_EVENT,
        &p,
        sizeof(arduino_usb_hid_event_data_t),
        portMAX_DELAY
    );
}
...
uint16_t tud_hid_get_report_cb(
    ...
    if (!res) {
        log_d(
            "instance: %u, report_id: %u, report_type: %s, reqlen: %u",
            instance,
            report_id,
            tinyusb_hid_device_report_types[report_type],
            reqlen
        );
    }
    return res;
...
void tud_hid_set_report_cb(
    ...
    if (!report_id && (!report_type || report_type == HID_REPORT_TYPE_OUTPUT)) {
        if (!tinyusb_on_set_output(0, buffer, bufsize) &&
            ...
            log_d(
                "instance: %u, report_id: %u, report_type: %s, bufsize: %u",
                instance,
                buffer[0],
                tinyusb_hid_device_report_types[HID_REPORT_TYPE_OUTPUT],
                bufsize - 1
            );
        }
    } else {
...
bool USBHID::ready(void) { return tud_hid_n_ready(0); }
...
void tud_hid_report_complete_cb(
    uint8_t instance, uint8_t const *report, tud_hid_report_complete_cb_len_t len
) {
    if (tinyusb_hid_device_input_sem) { xSemaphoreGive(tinyusb_hid_device_input_sem); }
}
...
void USBHID::onEvent(esp_event_handler_t callback) { onEvent(ARDUINO_USB_HID_ANY_EVENT, callback); }
